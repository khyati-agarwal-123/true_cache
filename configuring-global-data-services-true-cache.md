[Previous](true-cache-integration-global-data-services.md) [Next](true-
cache-restrictions-oracle-global-data-services.md) JavaScript must be
enabled to correctly display this content
  ...
  3. Configuring Global Data Services for True Cache

## 9.3 Configuring Global Data Services for True Cache

These are the basic steps to configure Oracle Global Data Services (GDS) for
True Cache.
...
Before you install any software, review the hardware, network, operating
system, and other software requirements for Linux.
...
The global service manager (GSM) is the central component of the GDS
framework, and it includes the `GDSCTL` command-line interface.

Install at least one GSM in each region. For high availability, include two
GSMs in each region.

To install GSMs, see the following topics in the Oracle Database Global Data
...
Create the GDS catalog on the primary database or a database outside the GDS
configuration.

For large-scale GDS configurations, Oracle recommends that the GDS catalog be
hosted outside the databases in the GDS configuration.
...
If you don't specify a region when setting up the catalog, then a default
region named `REGIONORA` is created. You can optionally add more regions
later.

### 9.3.6 Set Up Global Service Managers

Before you can start a global service manager (GSM), you need to register the
GSM with the GDS catalog.
...
If you didn't already set up regions when creating the GDS catalog, add a GDS
region for each data center.

For high availability, assign a buddy region for each region. Only one buddy
region is allowed for each region.
...
Create and start the global database application services for the primary
database and True Caches.

Note:

If the application uses the JDBC programming model, both the primary database
service and True Cache service names must be fully qualified with the domain
name (for example, `sales.example.com` and `sales_tc.example.com`). This is
because GDS has a default domain name and is different from the database's
`domain_name` parameter. This also limits the fully qualified service name to
a maximum of 64 characters.

Note:

The PDB name is required to create a service for a PDB. If you don't specify
the PDB name, the service is created in `CDB$ROOT`.
...
On the primary database, connect to the PDB first. The following example uses
the `sales.example.com` and `sales_tc.example.com` services.
...
For manual configuration options, see [Create Database Application Services on
the Primary Database](configuring-true-cache-database-application-services-
manually.md#GUID-1F990837-4B60-4119-8119-AFC87F77EC38 "You can associate one
primary database service with one True Cache service.").
...
The following example starts the services on the primary database and on all
True Caches:
...
To use global services with the JDBC `setReadOnly(TRUE)` flag, follow these
best practices.
...
Update the `tnsnames.ora` files for the primary database and True Caches based
on region.

Example 9-1 Primary Database

In this example, the `ADDRESS_LIST` points to the GSM in the region where the
primary database is located.
...
In this example, the first `ADDRESS_LIST` points to the GSM in the region
where the True Cache is located. The second `ADDRESS_LIST` points to the GSM
in the associated buddy region.
...
[← Previous](true-cache-integration-global-data-services.md)
